...
import { clientsDb, projectsDb, type ClientStatus, type ProjectStage } from '../services/storageDb'
...
const STATUS_LABELS: Record<ClientStatus, string> = {
  lead: 'Лид', active: 'В работе', paused: 'Пауза', done: 'Готово',
}
...
export default function ClientDetails() {
  ...
  const client = useMemo(() => id ? clientsDb.getById(id) : undefined, [id])
  const projects = useMemo(() => id ? projectsDb.getByClient(id) : [], [id])
  ...
  return (
    <div className="space-y-6">
      <button onClick={() => navigate('/app/clients')} className="text-sm text-gray-500 hover:text-gray-700">
        ← К списку клиентов
      </button>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <div className="flex items-start justify-between">
          ...
          <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${STATUS_COLORS[client.status]}`}>
                ...
                {projects.map(p => (
                  <tr key={p.id} className="hover:bg-gray-50 transition cursor-pointer" onClick={() => navigate(`/app/projects/${p.id}`)}>
                    ...
                    <td className="px-5 py-3">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${STAGE_COLORS[p.stage]}`}>{STAGE_LABELS[p.stage]}</span>
                    </td>
