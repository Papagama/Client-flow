...
import type { Task, TaskStatus, TaskPriority, Payment, PaymentStatus, Link, Comment } from '../services/storageDb'
...
const TASK_STATUS_LABELS: Record<TaskStatus, string> = {
  todo: 'К выполнению', doing: 'В процессе', done: 'Готово',
}
...
const TAB_LABELS: Record<string, string> = {
  tasks: 'Задачи', payments: 'Оплаты', links: 'Ссылки',
}
...
export default function ProjectDetails() {
  ...
  const project = useMemo(() => id ? projectsDb.getById(id) : undefined, [id, tick])
  const client = useMemo(() => project ? clientsDb.getById(project.clientId) : undefined, [project])
  const tasks = useMemo(() => id ? tasksDb.getByProject(id) : [], [id, tick])
  const payments = useMemo(() => id ? paymentsDb.getByProject(id) : [], [id, tick])
  const links = useMemo(() => id ? linksDb.getByProject(id) : [], [id, tick])

  if (!project) {
    return (
      <div className="text-center py-20 text-gray-400">
        Проект не найден.
        <button onClick={() => navigate('/app/projects')} className="ml-2 text-indigo-600 hover:underline">Назад</button>
      </div>
    )
  }

  function duplicateProject() {
    const newP = projectsDb.create({
    ...
    tasks.forEach(t => tasksDb.create({ projectId: newP.id, title: t.title, description: t.description, status: t.status, dueDate: t.dueDate, priority: t.priority }))
    payments.forEach(p => paymentsDb.create({ projectId: newP.id, amount: p.amount, description: p.description, date: p.date, status: p.status }))
    links.forEach(l => linksDb.create({ projectId: newP.id, label: l.label, url: l.url }))
    toast('Проект дублирован')
    navigate(`/app/projects/${newP.id}`)
  }
  ...
  function exportProject() {
    ...
    a.href = url
    a.download = `${project.title.replace(/\s+/g, '-').toLowerCase()}-report.json`
    a.click()
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Клиент: {client?.name ?? '—'}</p>
          </div>
          <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${STAGE_COLORS[project.stage] ?? ''}`}>
      ...
      <div className="flex gap-1 border-b border-gray-200">
        {(['tasks', 'payments', 'links'] as Tab[]).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2.5 text-sm font-medium border-b-2 transition -mb-px ${tab === t ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
            {TAB_LABELS[t]} ({t === 'tasks' ? tasks.length : t === 'payments' ? payments.length : links.length})
          </button>
        ))}
      </div>

      {tab === 'tasks' && <TasksTab tasks={tasks} projectId={project.id} refresh={refresh} toast={toast} />}
      {tab === 'payments' && <PaymentsTab payments={payments} projectId={project.id} refresh={refresh} toast={toast} />}
      {tab === 'links' && <LinksTab links={links} projectId={project.id} refresh={refresh} toast={toast} />}
    </div>
  )
}
...
function TasksTab({ tasks, projectId, refresh, toast }: { tasks: Task[]; projectId: string; refresh: () => void; toast: (m: string, t?: 'success' | 'error' | 'info', a?: { label: string; onClick: () => void }) => void }) {
  ...
  function validate() {
    ...
    if (!form.dueDate) e.dueDate = 'Укажите дату'
    return e
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (editing) { tasksDb.update(editing.id, form); toast('Задача обновлена') }
    else { tasksDb.create({ ...form, projectId }); toast('Задача создана') }
    refresh(); setModalOpen(false)
  }

  function remove(id: string) {
    const backup = tasksDb.getById(id)
    tasksDb.delete(id)
    refresh()
    toast('Задача удалена', 'success', backup ? {
      label: 'Отменить',
      onClick: () => { tasksDb.create({ projectId: backup.projectId, title: backup.title, description: backup.description, status: backup.status, dueDate: backup.dueDate, priority: backup.priority }); refresh() },
    } : undefined)
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-end">
        <button onClick={openCreate} className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">+ Добавить задачу</button>
      </div>

      {sorted.length === 0 ? (
        <p className="text-sm text-gray-400 py-4 text-center">Задач пока нет</p>
      ) : (
            ...
            <div key={t.id} className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3 hover:bg-gray-50 transition">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <button onClick={() => setExpandedTask(expandedTask === t.id ? null : t.id)} className="text-gray-400 hover:text-gray-600 shrink-0">
                    {expandedTask === t.id ? '▾' : '▸'}
                  </button>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select value={form.status} onChange={e => setForm(p => ({ ...p, status: e.target.value as TaskStatus }))} className={inputCls()}>
                <option value="todo">К выполнению</option><option value="doing">В процессе</option><option value="done">Готово</option>
              </select>
            </div>
            <div>
...
function CommentsSection({ taskId }: { taskId: string }) {
  const [comments, setComments] = useState<Comment[]>(() => commentsDb.getByTask(taskId))
  const [text, setText] = useState('')

  function addComment() {
    if (!text.trim()) return
    const author = localStorage.getItem('profile_name') ?? 'Админ'
    commentsDb.create({ taskId, text: text.trim(), author })
    setComments(commentsDb.getByTask(taskId))
    setText('')
  }
    ...
    <div className="border-t border-gray-100 px-5 py-3 bg-gray-50">
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 px-3 py-1.5 rounded-lg border border-gray-200 text-xs outline-none focus:ring-2 focus:ring-indigo-400"
        />
        <button onClick={addComment} disabled={!text.trim()} className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 text-white text-xs font-medium rounded-lg transition">
          Отправить
        </button>
      </div>
    </div>
...
function PaymentsTab({ payments, projectId, refresh, toast }: { payments: Payment[]; projectId: string; refresh: () => void; toast: (m: string, t?: 'success' | 'error' | 'info', a?: { label: string; onClick: () => void }) => void }) {
  ...
  function cycleStatus(p: Payment) {
    const next: Record<PaymentStatus, PaymentStatus> = { unpaid: 'partial', partial: 'paid', paid: 'unpaid' }
    paymentsDb.update(p.id, { status: next[p.status] })
    refresh()
    toast(`Статус → ${PAY_LABELS[next[p.status]]}`)
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (Object.keys(errs).length) { setErrors(errs); return }
    paymentsDb.create({ projectId, amount, description: form.description.trim(), date: form.date, status: form.status })
    setForm({ amount: '', description: '', date: '', status: 'unpaid' })
    setErrors({}); setShowForm(false); refresh()
    toast('Оплата добавлена')
  }

  function remove(id: string) {
    const all = paymentsDb.getAll()
    const backup = all.find(p => p.id === id)
    paymentsDb.delete(id)
    refresh()
    toast('Оплата удалена', 'success', backup ? {
      label: 'Отменить',
      onClick: () => { paymentsDb.create({ projectId: backup.projectId, amount: backup.amount, description: backup.description, date: backup.date, status: backup.status }); refresh() },
    } : undefined)
  }
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex gap-4 text-sm">
          <span className="text-gray-500">Всего: <span className="font-medium text-gray-800">${total.toLocaleString()}</span></span>
          <span className="text-gray-500">Оплачено: <span className="font-medium text-green-700">${paid.toLocaleString()}</span></span>
        </div>
        <button onClick={() => setShowForm(v => !v)} className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">
          {showForm ? 'Отмена' : '+ Добавить оплату'}
        </button>
      </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" step="0.01" value={form.amount} onChange={e => { setForm(p => ({ ...p, amount: e.target.value })); setErrors(p => ({ ...p, amount: '' })) }} className={inputCls(errors.amount)} placeholder="0.00" />
              {errors.amount && <p className="text-xs text-red-500 mt-1">{errors.amount}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Дата</label>
              <input type="date" value={form.date} onChange={e => { setForm(p => ({ ...p, date: e.target.value })); setErrors(p => ({ ...p, date: '' })) }} className={inputCls(errors.date)} />
              {errors.date && <p className="text-xs text-red-500 mt-1">{errors.date}</p>}
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Описание</label>
            <input value={form.description} onChange={e => { setForm(p => ({ ...p, description: e.target.value })); setErrors(p => ({ ...p, description: '' })) }} className={inputCls(errors.description)} placeholder="напр. Аванс за дизайн" />
            {errors.description && <p className="text-xs text-red-500 mt-1">{errors.description}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Статус</label>
            <select value={form.status} onChange={e => setForm(p => ({ ...p, status: e.target.value as PaymentStatus }))} className={inputCls()}>
              <option value="unpaid">Не оплачено</option><option value="partial">Частично</option><option value="paid">Оплачено</option>
            </select>
          </div>
          <div className="flex justify-end">
      ...
      {payments.length === 0 ? (
        <p className="text-sm text-gray-400 py-4 text-center">Оплат пока нет</p>
      ) : (
          ...
          {payments.map(p => (
            <div key={p.id} className="flex items-center justify-between px-5 py-3">
              <div>
                ...
                <p className="text-xs text-gray-400">{fmt(p.date)}</p>
              </div>
...
function LinksTab({ links, projectId, refresh, toast }: { links: Link[]; projectId: string; refresh: () => void; toast: (m: string, t?: 'success' | 'error' | 'info', a?: { label: string; onClick: () => void }) => void }) {
  const [label, setLabel] = useState('')
  const [url, setUrl] = useState('')

  function add() {
    if (!label.trim() || !url.trim()) return
    linksDb.create({ projectId, label: label.trim(), url: url.trim() })
    setLabel(''); setUrl('')
    refresh(); toast('Ссылка добавлена')
  }

  function remove(id: string) {
    const all = linksDb.getAll()
    const backup = all.find(l => l.id === id)
    linksDb.delete(id)
    refresh()
    toast('Ссылка удалена', 'success', backup ? {
      label: 'Отменить',
      onClick: () => { linksDb.create({ projectId: backup.projectId, label: backup.label, url: backup.url }); refresh() },
    } : undefined)
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {links.length === 0 ? (
        <p className="text-sm text-gray-400 py-4 text-center">Ссылок пока нет</p>
      ) : (
        <div className="bg-white rounded-xl shadow-sm divide-y divide-gray-100">
          {links.map(l => (
